...
const ChangePassword = () => {
  ...
  const handlePasswordChangeRequest = async () => {
    ...
    setMessage('');
    try {
      const { data } = await axios.post('http://localhost:4000/api/v1/Password-Change-Request', {
        Email: email,
        newPassword,
      });
  ...
  const handleResendOtp = async () => {
    ...
    setMessage('');
    try {
      const { data } = await axios.post('http://localhost:4000/api/v1/resend-otp', {
        email,
        type: 'password_reset',
      });
      if (data.success) {
  ...
  const handleVerifyOtp = async () => {
    ...
    setMessage('');
    try {
      const { data } = await axios.post('http://localhost:4000/api/v1/verify-otp', {
        email,
        otp,
        type: 'password_reset',
      });
