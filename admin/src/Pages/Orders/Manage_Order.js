...
const ManageOrder = () => {
  ...
  const fetchOrders = async (page) => {
    try {
      setLoading(true);
      const response = await axios.get("http://localhost:4000/api/v1/admin/get-all-order", {
        params: { page, limit: 6 },
      });
  ...
  const handleStatusChange = async (orderId, status) => {
    try {
      ...
      setLoading(true);
      const data = await axios.post("http://localhost:4000/api/v1/admin/change-order-status", { orderId, status });
      console.log(data)
      fetchOrders(currentPage);
    } catch (err) {
      console.log(err);
  ...
  const handleDeleteOrder = async (orderId) => {
    if (window.confirm("Are you sure you want to delete this order?")) {
      try {
        setLoading(true);
        await axios.delete(`http://localhost:4000/api/v1/admin/delete-order/${orderId}`);
        fetchOrders(currentPage);
